...
const onboardingData = [
  {
    ...
    subtitle: "Connect, Learn and grow with our vibrant community around the globe",
    image: require('../../assets/board1.png'),
  },
  {
    title: "Community",
    subtitle: "Join like-minded individuals, share ideas, and collaborate on projects",
    image: require('../../assets/board22.png'),
  },
  {
    title: "Collaborate and Create",
    subtitle: "Expand Your horizons in our inclusive and enriching learning community",
...
export default function OnboardingScreen({ onFinish, onSkip }) {
  ...
  const handleNextButtonPressIn = () => {
    buttonScale.value = withTiming(0.9, { duration: 100, easing: Easing.inOut(Easing.ease) });
  };
    ...
    if (title === "Endless Possibilities") {
      return (
        <Text style={styles.title}>
          <Text style={styles.title}>Endless</Text> <Text style={styles.greenTitle}>Possibilities</Text>
        </Text>
      );
    } else if (title === "Collaborate and Create") {
      return (
        <Text style={styles.title}>
          <Text style={styles.title}>Collaborate and</Text> <Text style={styles.greenTitle}>Create</Text>
        </Text>
      );
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {currentPage < onboardingData.length - 1 && ( // Conditional rendering for the "Skip" button
        <TouchableOpacity activeOpacity={0.5} style={styles.skipButton} onPress={handleSkip}>
        ...
        {onboardingData.map((item, index) => (
          <View key={index} style={styles.pageContainer}>
            ...
            <View style={styles.footer}>
              {currentPage < onboardingData.length - 1 && index === currentPage && (
